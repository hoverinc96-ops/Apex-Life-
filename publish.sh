...
bun run build
setsid nohup env DATABASE_URL="$DATABASE_URL" RESEND_API_KEY="$RESEND_API_KEY" UPSTASH_REDIS_REST_URL="$UPSTASH_REDIS_REST_URL" UPSTASH_REDIS_REST_TOKEN="$UPSTASH_REDIS_REST_TOKEN" DEEPGRAM_API_KEY="$DEEPGRAM_API_KEY" ELEVENLABS_API_KEY="$ELEVENLABS_API_KEY" GOOGLE_CLIENT_ID="$GOOGLE_CLIENT_ID" GOOGLE_CLIENT_SECRET="$GOOGLE_CLIENT_SECRET" bun run start > .run/server.log 2>&1 < /dev/null &
